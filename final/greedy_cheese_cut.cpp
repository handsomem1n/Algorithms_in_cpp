...
struct Cheese {
    ...
    Cheese(int v, int p) : volume(v), price(p) { // 구조체 constructor
...
int get_max_value(vector<Cheese> &cheeses, int max_volume) {
    ...
    for (auto &cheese : cheeses) {
        ...
        int take_volume = min(cheese.volume, max_volume); // 이 라인은 가져갈 치즈의 부피를 결정. 이는 현재 치즈의 부피와 남은 최대 부피 중 더 작은 값을 선택. 이렇게 하면, 치즈를 부분적으로만 가져갈 수 있고, 남은 부피를 초과하지 않도록 할 수 있음
