...
int cheese_no_cut(const vector<Cheese>& cheeses, int max_volume) {
    ...
    // dp[0][max_volume]은 아무 치즈도 고려하지 않는 초기 상태를 나타내므로, 치즈의 수보다 한 행 더 많이 필요하다고 함
    // dp[n][0]은 부피 제한이 0인 상태, 즉 아무것도 담을 수 없는 상태를 나타내므로 부피의
    // vector<int>(max_volume + 1, 0) : 각 행의 모든 값을 0으로 초기화함. 이는 처음에는 어떤 치즈도 선택하지 않았으며, 따라서 최대 가치는 0이라는 것을 나타냄.
